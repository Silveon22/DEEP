...
public class MobVolatileSlime extends MobSlime {
	private final boolean hasSlimeSplit;
	private int jumpDelay = 0;
	public MobVolatileSlime(World world) {
		super(world);
		this.textureIdentifier = NamespaceID.getPermanent("deep", "volatile_slime");
	...
	public MobVolatileSlime(World world, boolean isSplit) {
		super(world);
		this.textureIdentifier = NamespaceID.getPermanent("deep", "volatile_slime");
		this.heightOffset = 0.0F;
	...
	protected void updateAI() {
		...
		if (this.onGround && this.jumpDelay-- <= 0) {
			...
			this.squish = 1.8F;
			this.moveStrafing = 2.0F - this.random.nextFloat() * 2F;
			this.moveForward = (float)this.getSlimeSize() * 3;
		} else {
	...
	public void remove() {
		...
		if (!this.world.isClientSide && size > 1 && this.getHealth() <= 0) {
			...
			for(int j = 0; j < splitCount; ++j) {
				...
				MobVolatileSlime mobVolatileSlime = new MobVolatileSlime(this.world, true);
				mobVolatileSlime.setSlimeSize(size / 2);
				mobVolatileSlime.moveTo(this.x + (double)f, this.y + (double)0.5F, this.z + (double)f1, this.random.nextFloat() * 360.0F, 0.0F);
				this.world.entityJoinedWorld(mobVolatileSlime);
			}
		}
	...
	public void playerTouch(Player player) {
		...
		if (this.canEntityBeSeen(player) && (double)this.distanceTo(player) < 1 * (double)i) {
			this.world.playSoundAtEntity((Entity)null, this, "random.explode", 1.0F, (this.random.nextFloat() - this.random.nextFloat()) * 0.2F + 1.0F);
			player.fireHurt();
			player.hurt((Entity)null, 10, DamageType.BLAST);
	...
	protected String getDeathSound() {return "mob.creeperdeath";}
