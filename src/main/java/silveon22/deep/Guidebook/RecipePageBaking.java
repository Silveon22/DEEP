...
public class RecipePageBaking extends RecipePage<RecipeEntryBrickOven> {
	public static final int RECIPES_PER_PAGE = 6;
	public List<SlotGuidebook> slots;
	public Map<RecipeEntryBrickOven, List<SlotGuidebook>> map;
	private final TooltipElement tooltipElement;
	private final ItemElement itemElement;
	private static final Minecraft mc = Minecraft.getMinecraft();
	private static long ticks = 0L;

	public RecipePageBaking(GuidebookSection section, ArrayList<RecipeEntryBrickOven> recipes) {
		super(section);
		...
		for(RecipeEntryBrickOven recipe : recipes) {
	...
	protected void renderBackground(TextureManager re, int x, int y) {
		...
		for(int i = 1; i <= this.recipes.size(); ++i) {
			RecipeEntryBrickOven recipe = (RecipeEntryBrickOven)this.recipes.get(i - 1);
